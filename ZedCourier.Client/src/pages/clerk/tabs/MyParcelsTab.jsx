...
export default function MyParcelsTab() {
  ...
  const handleSendPin = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5076/api/v1/parcel/${selectedParcel.id}/send-delivery-pin`, {
        method: 'POST',
  ...
  const handleRegeneratePin = async () => {
    ...
    try {
      const res  = await fetch(`http://localhost:5076/api/v1/parcel/${pinParcel.id}/regenerate-pin`, {
