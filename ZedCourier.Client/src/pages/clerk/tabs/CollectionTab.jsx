...
export default function CollectionTab() {
  ...
  const fetchTrackingLogs = async (parcelId) => {
    try {
      const res = await fetch(`http://localhost:5076/api/v1/parcel/${parcelId}`, {
        headers: { Authorization: `Bearer ${token()}` }
      })
  ...
  const sendReceiptEmail = async (collection) => {
    try {
      const res = await fetch('http://localhost:5076/api/v1/notification/send-receipt', {
        method: 'POST',
  ...
  const notifySender = async (collection) => {
    try {
      const res = await fetch('http://localhost:5076/api/v1/notification/notify-sender', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` },
        body: JSON.stringify({
  ...
  const submitCollection = async (collection, isFromQueue = false) => {
    try {
      ...
      // Submit collection
      const updateRes = await fetch(`http://localhost:5076/api/v1/parcel/${parcel.id}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` },
        body: JSON.stringify({
          newStatus: 'Collected',
