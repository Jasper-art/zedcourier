...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <Box sx={{ backgroundColor: '#0d1b2e', border: '1px solid #1a2f4a', borderRadius: 2, p: 1.5 }}>
      ...
      <Typography variant="body2"   sx={{ color: '#4caf50', fontWeight: 700 }}>K {payload[0].value?.toFixed(2)}</Typography>
    </Box>
  )
}
...
export default function OverviewTab({ finance = {} }) {
  const [parcels,  setParcels]  = useState([])
  const [loading,  setLoading]  = useState(true)
  const [error,    setError]    = useState(null)

  const { summary, daily = [], byBranch = [] } = finance

  useEffect(() => {
    api.getParcels()
      .then(p => setParcels(Array.isArray(p) ? p : []))
      .catch(err => setError(err.message))
      .finally(() => setLoading(false))
  }, [])

  if (loading || finance.loading) return (
  ...
  const topRoutes = Object.entries(routeMap)
    ...
    .map(([key, count]) => {
      ...
      return { origin: branchMap[orig] ?? 'Unknown', destination: branchMap[dest] ?? 'Unknown', count }
    })
  ...
  return (
    <Box>
            ...
            {chartData.length === 0
              ...
              : <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={chartData}>
                    ...
                    <YAxis tick={{ fill: '#506680', fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={v => `K${v}`} />
                    <Tooltip content={<CustomTooltip />} />
                    <Line type="monotone" dataKey="revenue" stroke="#4fc3f7"
                      strokeWidth={2.5} dot={{ fill: '#4fc3f7', r: 3 }} activeDot={{ r: 5, fill: '#4fc3f7' }} />
                  </LineChart>
                </ResponsiveContainer>
            }
          </CardContent>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '5fr 7fr' }, gap: 2, mb: 3 }}>
              ...
              <Box key={s.label} sx={{ mb: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  ...
                  <Typography variant="body2" sx={{ color: '#f0f4ff', fontWeight: 600 }}>
                    {s.value} <span style={{ color: '#506680', fontSize: 11 }}>({Math.round((s.value / total) * 100)}%)</span>
                  </Typography>
                </Box>
        ...
        <Card>
          <CardContent>
            <Typography variant="subtitle1" sx={{ color: '#f0f4ff', fontWeight: 600, mb: 2 }}>Top Routes</Typography>
            {topRoutes.length === 0
              ? <Typography sx={{ color: '#506680' }}>No route data yet.</Typography>
              : topRoutes.map((r, i) => (
                <Box key={i} sx={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  p: 1.5, mb: 1.5, backgroundColor: '#0a1628', borderRadius: 2, border: '1px solid #1a2f4a'
                }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Typography sx={{ color: '#ce93d8', fontWeight: 700, fontSize: 18 }}>#{i + 1}</Typography>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      <LocationOnIcon   sx={{ color: '#4fc3f7', fontSize: 14 }} />
                      <Typography sx={{ color: '#f0f4ff', fontSize: 13, fontWeight: 600 }}>{r.origin}</Typography>
                      <ArrowForwardIcon sx={{ color: '#506680', fontSize: 14 }} />
                      <LocationOnIcon   sx={{ color: '#4caf50', fontSize: 14 }} />
                      <Typography sx={{ color: '#f0f4ff', fontSize: 13, fontWeight: 600 }}>{r.destination}</Typography>
                    </Box>
                  </Box>
            ...
            {byBranch.map((b, i) => (
              <Box key={i} sx={{ mb: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  ...
                  <Box sx={{ textAlign: 'right' }}>
                    <Typography variant="body2" sx={{ color: '#4caf50', fontWeight: 700 }}>K {b.revenue?.toFixed(2)}</Typography>
                    <Typography variant="caption" sx={{ color: '#506680' }}>{b.parcels} parcels</Typography>
                  </Box>
                </Box>
                <LinearProgress variant="determinate" value={(b.revenue / maxBranchRevenue) * 100}
      ...
      <Card>
        <CardContent>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="subtitle1" sx={{ color: '#f0f4ff', fontWeight: 600 }}>Recent Parcels</Typography>
