...
export default function ScannerTab() {
  ...
  const fetchParcelDetails = async waybillNum => {
    const trackRes  = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/tracking/${waybillNum.trim()}`)
    const trackData = await trackRes.json()
    if (!trackRes.ok) throw new Error(trackData.error || 'Waybill not found')

    const parcelRes  = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/parcel/${trackData.parcelId}`, {
  ...
  const confirmScan = async () => {
    ...
    try {
      if (!isOnline) {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/parcel/${selectedParcel.parcelId}/status`, {
          method: 'PUT',
  ...
  const syncOfflineScans = async () => {
    for (const scan of offlineScans) {
      try {
        await fetch(`${import.meta.env.VITE_API_URL}/api/v1/parcel/${scan.parcelId}/status`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` },
  ...
  const handleBatchScan = async () => {
    ...
    for (const wb of waybills) {
      try {
        ...
        if (isOnline) {
          const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/parcel/${data.parcelId}/status`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` },
            body: JSON.stringify({ newStatus: STATUS_MAP[action], notes: NOTE_MAP[action] })
          })
